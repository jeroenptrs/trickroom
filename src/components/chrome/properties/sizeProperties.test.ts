...
import { applySizeInput, inputToSizeUtility, readSizeValue } from "./sizePropertiesController";
...
describe("sizePropertiesController", () => {
	describe("inputToSizeUtility", () => {
		it("builds a utility body from prefix and input", () => {
			...
			expect(inputToSizeUtility("max-w", "(--my-var)")).toBe("max-w-(--my-var)");
		});
	...
	describe("applySizeInput", () => {
		it("sets a width utility and preserves other classes", () => {
			const result = applySizeInput("h-4 unknown-card", opts, "size.width", "w", "full");
...
describe("size domain — shared styleSectionController helpers", () => {
	it("sets width without touching height", () => {
		const next = applyStyleUtility("h-4 unknown-cls", opts, "size.width", "w-full");
		expect(next).toBe("h-4 unknown-cls w-full");
	});

	it("replaces width slot exactly, leaving height and unknown classes intact", () => {
		const next = applyStyleUtility("w-4 h-8 text-sm", opts, "size.width", "w-full");
		expect(next).toBe("w-full h-8 text-sm");
	});

	it("clears width without touching height or unrelated classes", () => {
		expect(clearStyleProperty("w-4 h-8 unknown", opts, "size.width")).toBe("h-8 unknown");
	});

	it("sets height independently of width", () => {
		const next = applyStyleUtility("w-full h-4", opts, "size.height", "h-screen");
		expect(next).toBe("w-full h-screen");
	});

	it("min-w and max-w occupy separate slots", () => {
		const afterMin = applyStyleUtility("max-w-lg unknown", opts, "size.min-width", "min-w-0");
		expect(afterMin).toBe("max-w-lg unknown min-w-0");
		const afterMax = applyStyleUtility(afterMin, opts, "size.max-width", "max-w-xl");
	...
	it("grow and shrink are separate slots", () => {
		const next = applyStyleUtility("grow shrink-0 w-4", opts, "size.grow", "grow-0");
		expect(next).toBe("grow-0 shrink-0 w-4");
	});

	it("unknown classes round-trip unchanged when setting a size property", () => {
		const next = applyStyleUtility("unknown-card cool-class", opts, "size.width", "w-4");
		expect(next).toBe("unknown-card cool-class w-4");
	});

	it("reads grow value correctly", () => {
		expect(styleValueText(getStyleIntent("grow w-4", opts, "size.grow"))).toBe("1");
		expect(styleValueText(getStyleIntent("grow-0 w-4", opts, "size.grow"))).toBe("0");
		expect(getStyleIntent("w-4", opts, "size.grow")).toBeUndefined();
	});

	it("reads flex value correctly", () => {
		expect(styleValueText(getStyleIntent("flex-1", opts, "size.flex"))).toBe("1");
		expect(styleValueText(getStyleIntent("flex-auto", opts, "size.flex"))).toBe("auto");
		expect(styleValueText(getStyleIntent("flex-none", opts, "size.flex"))).toBe("none");
	});
});
