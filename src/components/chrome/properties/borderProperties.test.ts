...
describe("border domain — styleSectionController helpers", () => {
	describe("border-width", () => {
		it("sets border (DEFAULT) without touching other classes", () => {
			const next = applyStyleUtility("text-sm unknown", opts, "border.border-width", "border");
			expect(next).toBe("text-sm unknown border");
		});

		it("replaces border width slot exactly", () => {
			const next = applyStyleUtility("border-2 text-sm", opts, "border.border-width", "border-4");
			expect(next).toBe("border-4 text-sm");
		});

		it("replaces bare 'border' with 'border-2'", () => {
			const next = applyStyleUtility("border rounded-md", opts, "border.border-width", "border-2");
			expect(next).toBe("border-2 rounded-md");
		});

		it("clears border-width without affecting border-style or radius", () => {
			expect(
				clearStyleProperty("border-2 border-solid rounded-lg", opts, "border.border-width"),
			).toBe("border-solid rounded-lg");
		});

		it("reads border-width value: DEFAULT for bare border", () => {
			expect(styleValueText(getStyleIntent("border text-sm", opts, "border.border-width"))).toBe(
				"DEFAULT",
			);
		});

		it("reads border-width value: scale for border-2", () => {
			expect(styleValueText(getStyleIntent("border-2", opts, "border.border-width"))).toBe("2");
		});
	});

	describe("border-style", () => {
		it("sets border-style without touching border-width", () => {
			const next = applyStyleUtility("border-2 unknown", opts, "border.border-style", "border-dashed");
			expect(next).toBe("border-2 unknown border-dashed");
		});

		it("replaces border-style exactly", () => {
			const next = applyStyleUtility("border-solid border-2", opts, "border.border-style", "border-dotted");
			expect(next).toBe("border-dotted border-2");
		});

		it("border-width and border-style are separate slots", () => {
			const withWidth = applyStyleUtility("unknown", opts, "border.border-width", "border-2");
			const withStyle = applyStyleUtility(withWidth, opts, "border.border-style", "border-dashed");
	...
	describe("radius", () => {
		it("sets bare overall radius", () => {
			const next = applyStyleUtility("border-2", opts, "border.radius", "rounded");
		...
		it("sets overall radius", () => {
			const next = applyStyleUtility("border-2", opts, "border.radius", "rounded-md");
			expect(next).toBe("border-2 rounded-md");
		});
		...
		it("clears radius without touching border-width", () => {
			expect(clearStyleProperty("border-2 rounded-lg", opts, "border.radius")).toBe("border-2");
		});

		it("reads radius keyword value", () => {
			expect(styleValueText(getStyleIntent("rounded-md border-2", opts, "border.radius"))).toBe("md");
			expect(styleValueText(getStyleIntent("rounded-full", opts, "border.radius"))).toBe("full");
	...
	describe("divide utilities", () => {
		it("sets divide-x without touching divide-y", () => {
			const next = applyStyleUtility("divide-y-2 unknown", opts, "border.divide-x-width", "divide-x");
			expect(next).toBe("divide-y-2 unknown divide-x");
		});

		it("divide-x and divide-y are separate slots", () => {
			const withX = applyStyleUtility("unknown", opts, "border.divide-x-width", "divide-x-2");
			const withY = applyStyleUtility(withX, opts, "border.divide-y-width", "divide-y-4");
			expect(withY).toBe("unknown divide-x-2 divide-y-4");
		});

		it("divide does not collide with border-width", () => {
			const next = applyStyleUtility("border-2 divide-x", opts, "border.divide-x-width", "divide-x-4");
			expect(next).toBe("border-2 divide-x-4");
		});

		it("divide-x DEFAULT value read correctly", () => {
			expect(
				styleValueText(getStyleIntent("divide-x border-2", opts, "border.divide-x-width")),
			).toBe("DEFAULT");
		});
	});
