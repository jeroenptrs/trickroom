...
describe("structurePropertiesController", () => {
	...
	it("rejects arbitrary bracket values except on supported structure properties", () => {
		...
		expect(structureUtility("structure.columns", "[3rem]")).toBe("columns-[3rem]");
	});

	it("reads structure values and round-trips columns without losing unknown classes", () => {
		const input = "box-content visible float-end break-inside-avoid columns-3 custom-struct";
		expect(readStructureValue(input, opts, "structure.box-sizing")).toBe("content");
		expect(readStructureValue(input, opts, "structure.visibility")).toBe("visible");
		expect(readStructureValue(input, opts, "structure.float")).toBe("end");
		expect(readStructureValue(input, opts, "structure.break-inside")).toBe("avoid");
		...
		expect(styleValueText(getStyleIntent(next, opts, "structure.columns"))).toBe("2");
	});
});
